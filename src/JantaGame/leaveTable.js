...
module.exports.leaveTable = async (requestData, client) => {
    ...
    if (typeof client.tbid == "undefined" || typeof client.uid == "undefined" || typeof client.seatIndex == "undefined") {
        commandAcions.sendDirectEvent(client.sck, CONST.LEAVETABLESJANTA, requestData, false, "User session not set, please restart game!");
        return false;
    }
    ...
    let tb = await JantaTables.findOne(wh, {});
    ...
    if (tb.activePlayer == 2 && tb.gameState == "JantaGameStartTimer") {
    ...
    let tbInfo = await JantaTables.findOneAndUpdate(wh, updateData, { new: true });
...
module.exports.manageOnUserLeave = async (tb, client) => {
    ...
    if (tb.gameState == "RoundStated" || tb.gameState == "CollectBoot") {
        ...
        if (playerInGame.length == 0 && tb.activePlayer == 0) {
            ...
            await JantaTables.deleteOne(wh);
...
module.exports.leaveSingleUser = async (tbid) => {
    ...
    const tabInfo = await JantaTables.findOne(wh1, {}).lean();
