...
const userReconnect = async (payload, socket) => {
  try {
    ...
    const disconnTable = await findDisconnectTable(payload.playerId, JantaTables);
...
const updateRejoinStatus = async (payload, table) => {
  try {
    ...
    if (table) {
      ...
      let tabInfo = await JantaTables.findOne(wh, project);
      ...
      tabInfo = await JantaTables.findOneAndUpdate(upWh, updateData, {
        new: true,
      });
