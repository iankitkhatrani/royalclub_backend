...
module.exports.roundStarted = async (tbid) => {
    try {
        ...
        let tabInfo = await JantaTables.findOne(wh, project).lean();
        ...
        const tb = await JantaTables.findOneAndUpdate(wh, update, { new: true });
...
module.exports.userTurnExpaire = async (tbid) => {
    try {
        ...
        const upRes = await JantaTables.findOneAndUpdate(whPlayer, update, { new: true });
        ...
        if (userDrop) {
            ...
            let taabInfo = await JantaTables.findOne(wh1, project1).lean();
