...
const gamePlayActions = require("./gamePlayLudo");
...
module.exports.roundStarted = async (tbid) => {
    try {
        ...
        let tabInfo = await playingLudo.findOne(wh, project).lean();
        ...
        const tb = await playingLudo.findOneAndUpdate(wh, update, { new: true });
...
module.exports.userTurnExpaire = async (tbid) => {
    try {
        ...
        const upRes = await playingLudo.findOneAndUpdate(whPlayer, update, { new: true });
        ...
        if (userDrop) {
            ...
            let taabInfo = await playingLudo.findOne(wh1, project1).lean();
