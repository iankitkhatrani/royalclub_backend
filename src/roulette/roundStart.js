...
module.exports.roundStarted = async (tbid) => {
    try {
        ...
        let tabInfo = await RouletteTables.findOne(wh, project).lean();
        ...
        const tb = await RouletteTables.findOneAndUpdate(wh, update, { new: true });
...
module.exports.userTurnExpaire = async (tbid) => {
    try {
        ...
        const upRes = await RouletteTables.findOneAndUpdate(whPlayer, update, { new: true });
        ...
        if (userDrop) {
            ...
            let taabInfo = await RouletteTables.findOne(wh1, project1).lean();
